...
for i in range((today - begin).days):
    ...
    util_device = vel_util.get_util_by_device(str(sd), str(ed))

    ts = datetime.datetime.combine(ed, datetime.time.min).timestamp()
    actions = []
...
#        print(json.dumps(util))
    ...
    if len(actions) > 0:
        ...
        print('Device daily utilization have been imported. Date: ' + str(sd))
